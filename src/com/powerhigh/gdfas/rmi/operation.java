...
public class operation {
  ...
  private static parse p1 = null;
  private static parse p2 = null;
  private static parse p3 = null;
  private static Integer frontid;
  //加载日志
  private static final String resource = "log4j.properties";
  private static Category cat =
      Category.getInstance(com.powerhigh.gdfas.rmi.operation.class);
//  static {
...
//      p = (parse)ctx.getBean("myServiceClient");
      ...
      if(null==frontid||0==frontid){
    	  ...
    	  if(p1==null){
  			ctx = new ClassPathXmlApplicationContext("epfront-client1.xml");
  			p1 = (parse) ctx.getBean("myServiceClient");
  	      }
    	  return p1;
      }else if(2==frontid){
    	  if(p2==null){
  			ctx = new ClassPathXmlApplicationContext("epfront-client2.xml");
  			p2 = (parse) ctx.getBean("myServiceClient");
  	      }
    	  return p2;
      }else if(3==frontid){
    	  if(p3==null){
  			ctx = new ClassPathXmlApplicationContext("epfront-client3.xml");
  			p3 = (parse) ctx.getBean("myServiceClient");
  	      }
    	  return p3;
      }else{
    	  if(p==null){
